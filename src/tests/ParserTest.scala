...
class ParserTest extends FunSuite {
  ...
  test("Funções: Fatorial Multi Parâmetros" ){
    ...
    var expected = Blk(Bind(Id("z"),Ref(Num(1.0))),Blk(BindAbs(Id("f"),Abs(IdSeq(Id("x"),IdSeq(Id("y"),Id("z"))),Blk(Bind(Id("y"),Ref(Id("x"))),Loop(Not(Lt(Id("y"),Num(1.0))),CSeq(Assign(Id("z"),Mul(Id("z"),Id("y"))),Assign(Id("y"),Sub(Id("y"),Num(1.0)))))))),Call(Id("f"),ExpSeq(Num(10.0),ExpSeq(Sum(Num(3.0),Num(5.0)),Or(Bool(true),Bool(false)))))))
    assert(result === expected)
  }
